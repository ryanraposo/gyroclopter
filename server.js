...
function getCertificates() {
    const keyPath = path.join(CONFIG.APP_DIR, 'key.pem');
    const certPath = path.join(CONFIG.APP_DIR, 'cert.pem');
...
<body>
    ...
    <script>
        ...
        function handleOrientation(e) {
            ...
            let dx = gamma - state.lastGamma;
            let dy = beta - state.lastBeta;

            // Handle wrap-around
            if (dx > 180) dx -= 360;
            else if (dx < -180) dx += 360;

            const finalDx = Math.round(dx * state.sensitivity);
            const finalDy = Math.round(dy * state.sensitivity);

            if (finalDx !== 0 || finalDy !== 0) {
                send({ type: 'move', dx: finalDx, dy: finalDy });
            }
        ...
        let lastY = 0;
        els.scrollPad.ontouchstart = (e) => { e.preventDefault(); lastY = e.touches[0].clientY; };
